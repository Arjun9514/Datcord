...
class sf:
    ...
    #Mute client
    def mute(target="", time=""):
        sf.prnt(f'{target} is muted for {time}',"o")
        sf.broadcast(f'{target} is muted for {time}')
        muted.append([target,time[:-1],time[-1:]])
        timer = threading.Thread(target=sf.timer, args=(target,float(time[:-1]),time[-1:]))
        timer.start()
