...
Userdata_path = r"H:\Datcord\New\Data\UserData.csv"
# Chatdata_path = r"H:\Datcord\New\Data\ChatData.csv"

# Database
database = pd.read_csv(Userdata_path)
...
class sf:
    ...
    #Broadcast message 
    def broadcast(message="",clt=None):
        for client in memory["clients"]:
            if clt != client:
                client.send(message.encode('ascii'))
        sf.save_message(message)
        sf.server_log(message)
...
/stp                      to stop server
            ...
            while(True):
                ...
                if(condn == "UP"):
                    ...
                    if connect == True:
                        ...
                        database =  database.append({"Username":username,"Password":pwd,"Muted":[False,"0"],"Banned":False},ignore_index=True)
                    ...
                    if connect == True:
                        ...
                        #chat history for the client
                        messageHistory = pd.read_csv(r'H:\Datcord\New\Data\message_history.csv')
                        tail = messageHistory.tail(50)
                        tail_string = tail.to_string(index = False, header = False)
                        tail_split = tail_string.split('\n')
                        for somemsg in tail_split:
                            client.send(f'{somemsg}'.strip().encode('ascii'))
                            client.send(' \n'.encode('ascii'))
    ...
    def shtdwn(tmr=0):
        ...
        if tmr == 0:
            ...
            sf.broadcast(f'-offline-')
        else:
            sf.broadcast(f'-shutdown-')
    ...
    def svbk():
        ...
        database.to_csv(Userdata_path,index=False)
        memory["saving"] = False
    ...
    #Save Messages
    def save_message(smsg):
        messageHistory = pd.read_csv(r'H:\Datcord\New\Data\message_history.csv')
        dont_save = ['-offline-']
        for i in dont_save:
            if smsg != i:
                new_row = {'Message':smsg}
                messageHistory.loc[len(messageHistory)] = new_row
                messageHistory.to_csv(r'H:\Datcord\New\Data\message_history.csv', index = False)

    #Server Log
    def server_log(smsg):
        log = pd.read_csv(r'H:\Datcord\New\Data\log.csv')
        new_row = {'Message':smsg}
        log.loc[len(log)] = new_row
        log.to_csv(r'H:\Datcord\New\Data\log.csv', index = False)
