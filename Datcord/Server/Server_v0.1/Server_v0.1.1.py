...
#dictionary
memory = {"clients":[],"usernames":[],"muted":[],"banned":[],"kicked":False,"saving":False}
...
class sf:
    ...
    def load():
        ...
        for d in (w:=list(database["Banned"].values)):
            if d == True:
                memory["banned"].append(database["Username"][w.index(d)])
    ...
    def dm(snder="",target="",msg=""):
        if target == "svr":
            ...
            i = memory["usernames"].index(snder)
            ...
            for un in memory["usernames"]:
                if target == un:
                    ...
                    memory["clients"][n].send(message.encode('ascii'))
                    sent = True
                    break
                else:
                    n += 1
    ...
    def stats():
        ...
        for un in memory["username"]:
            sf.prnt(un+"   ",cr="p",ed="")
        print()
        sf.prnt("Muted Clients:","p")
        for un in memory["muted"]:
            sf.prnt(un+"   ",cr="p",ed="")
        print()
        sf.prnt("Banned Clients:","p")
        for un in memory["banned"]:
            sf.prnt(un+"   ",cr="p",ed="")
        print()
    ...
    def kick(target=""):
        ...
        if memory["kicked"] == True:
            username = memory["username"][n]
            memory["username"].remove(username)
            client = memory["clients"][n]
            ...
            while(True):
                ...
                if(rcvdat == "-SIGNUP-"):
                    ...
                    if connect == True:
                        ...
                        memory["username"].append(username)
            ...
            while True:
                ...
                try:
                    ...
                    if memory["kicked"] == True:
                        ...
                        index = memory["clients"].index(client)
                        memory["clients"].remove(client)
                        client.close()
                        username = memory["username"][index]
